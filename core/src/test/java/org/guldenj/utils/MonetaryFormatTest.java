...
import static org.guldenj.core.Coin.CENT;
import static org.guldenj.core.Coin.COIN;
import static org.guldenj.core.Coin.SATOSHI;
import static org.guldenj.core.Coin.ZERO;
...
public class MonetaryFormatTest {
    ...
    private static final MonetaryFormat NO_CODE = MonetaryFormat.NLG.noCode();
    ...
    public void standardCodes() throws Exception {
        assertEquals("NLG 0.00", MonetaryFormat.NLG.format(Coin.ZERO).toString());
        assertEquals("mNLG 0.00", MonetaryFormat.MNLG.format(Coin.ZERO).toString());
        assertEquals("µNLG 0", MonetaryFormat.UNLG.format(Coin.ZERO).toString());
    }

    @Test
    public void customCode() throws Exception {
        assertEquals("dNLG 0", MonetaryFormat.UNLG.code(1, "dNLG").shift(1).format(Coin.ZERO).toString());
    }
    ...
    public void noCode() throws Exception {
        assertEquals("0", MonetaryFormat.UNLG.noCode().shift(0).format(Coin.ZERO).toString());
        // Ensure that inserting a code after codes are wiped, works
        assertEquals("dNLG 0", MonetaryFormat.UNLG.noCode().code(1, "dNLG").shift(1).format(Coin.ZERO).toString());
    }

    @Test
    public void codeOrientation() throws Exception {
        assertEquals("NLG 0.00", MonetaryFormat.NLG.prefixCode().format(Coin.ZERO).toString());
        assertEquals("0.00 NLG", MonetaryFormat.NLG.postfixCode().format(Coin.ZERO).toString());
    }

    @Test
    public void codeSeparator() throws Exception {
        assertEquals("NLG@0.00", MonetaryFormat.NLG.codeSeparator('@').format(Coin.ZERO).toString());
    }

    @Test(expected = NumberFormatException.class)
    public void missingCode() throws Exception {
        MonetaryFormat.UNLG.shift(1).format(Coin.ZERO);
    }
    ...
    public void parse() throws Exception {
        ...
        assertEquals(Coin.MILLICOIN, MonetaryFormat.MNLG.parse("1"));
        assertEquals(Coin.MILLICOIN, MonetaryFormat.MNLG.parse("1.0"));
        assertEquals(Coin.MILLICOIN, MonetaryFormat.MNLG.parse("01.0000000000"));
        assertEquals(Coin.MILLICOIN, MonetaryFormat.MNLG.positiveSign('+').parse("+1.0"));
        assertEquals(Coin.MILLICOIN.negate(), MonetaryFormat.MNLG.parse("-1"));
        assertEquals(Coin.MILLICOIN.negate(), MonetaryFormat.MNLG.parse("-1.0"));

        assertEquals(Coin.MICROCOIN, MonetaryFormat.UNLG.parse("1"));
        assertEquals(Coin.MICROCOIN, MonetaryFormat.UNLG.parse("1.0"));
        assertEquals(Coin.MICROCOIN, MonetaryFormat.UNLG.parse("01.0000000000"));
        assertEquals(Coin.MICROCOIN, MonetaryFormat.UNLG.positiveSign('+').parse("+1.0"));
        assertEquals(Coin.MICROCOIN.negate(), MonetaryFormat.UNLG.parse("-1"));
        assertEquals(Coin.MICROCOIN.negate(), MonetaryFormat.UNLG.parse("-1.0"));
