...
import org.guldenj.utils.BlockFileLoader;
...
public abstract class AbstractFullPrunedBlockChainTest {
    ...
    public void skipScripts() throws Exception {
        ...
        Block rollingBlock = params.getGenesisBlock().createNextBlockWithCoinbase(outKey.getPubKey(), null);
        chain.add(rollingBlock);
        TransactionOutput spendableOutput = rollingBlock.getTransactions().get(0).getOutput(0);
        for (int i = 1; i < params.getSpendableCoinbaseDepth(); i++) {
            rollingBlock = rollingBlock.createNextBlockWithCoinbase(outKey.getPubKey(), null);
            chain.add(rollingBlock);
        }

        rollingBlock = rollingBlock.createNextBlock(null, null);
    ...
    public void testUTXOProviderWithWallet() throws Exception {
        ...
        // Create 1 NLG spend to a key in this wallet (to ourselves).
        ...
        // Create another spend of 1/2 the value of NLG we have available using the wallet (store coin selector).
        ...
        // The availbale balance should be the 0 (as we spent the 1 NLG that's pending) and estimated should be 1/2 - fee NLG
        assertEquals("Available balance is incorrect", Coin.ZERO, wallet.getBalance(Wallet.BalanceType.AVAILABLE));
