...
public class Block extends Message {
     ...
     *
     * <p>The half-life is controlled by {@link org.guldenj.core.NetworkParameters#getSubsidyDecreaseBlockCount()}.
     * </p>
     */
    ...
    /**
     * Calculates the scrypt block hash by serializing the block and hashing the
     * resulting bytes.
     */
    ...
    /**
     * Returns the scrypt hash of the block (which for a valid, solved block should be below the target) in the form seen on
    ...
    public BigInteger getDifficultyTargetAsInteger() throws VerificationException {
        ...
        if (target.signum() <= 0 || target.compareTo(params.maxTarget) > 0)
            throw new VerificationException("Difficulty target is bad: " + Long.toString(difficultyTarget) + " - " + target.toString() + " - " + params.maxTarget.toString());
     ...
     * {@link org.guldenj.core.Block#getDifficultyTargetAsInteger()}. Note that this is <b>not</b> the same as
     ...
     */
    @VisibleForTesting
    public Block createNextBlock(Address to, long time, BlockStore blockStore) {
        return createNextBlock(to, null, time, pubkeyForTesting, FIFTY_COINS, blockStore);
    }
    ...
    Block createNextBlock(@Nullable Address to, @Nullable TransactionOutPoint prevOut, long time,
                          byte[] pubKey, Coin coinbaseValue, BlockStore blockStore) {
    ...
    public Block createNextBlock(@Nullable Address to, TransactionOutPoint prevOut, BlockStore blockStore) {
        return createNextBlock(to, prevOut, getTimeSeconds() + 5, pubkeyForTesting, FIFTY_COINS, blockStore);
    }

    @VisibleForTesting
    public Block createNextBlock(@Nullable Address to, Coin value, BlockStore blockStore) {
        return createNextBlock(to, null, getTimeSeconds() + 5, pubkeyForTesting, value, blockStore);
    }
    ...
    @VisibleForTesting
    public Block createNextBlockWithCoinbase(byte[] pubKey, Coin coinbaseValue, BlockStore blockStore) {
        return createNextBlock(null, null, Utils.currentTimeSeconds(), pubKey, coinbaseValue, blockStore);
    }

    /**
     * Create a block sending 50NLG as a coinbase transaction to the public key specified.
     * This method is intended for test use only.
     */
    @VisibleForTesting
    Block createNextBlockWithCoinbase(byte[] pubKey, BlockStore blockStore) {
        return createNextBlock(null, null, Utils.currentTimeSeconds(), pubKey, FIFTY_COINS, blockStore);
    }
