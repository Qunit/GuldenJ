 ...
 * guldenj for a specific network. You can get an instance of this class through calling {@link #get()}.</p>
 *
...
public class Context {
    ...
    public Context(NetworkParameters params) {
        log.info("Creating guldenj {} context.", VersionMessage.BITCOINJ_VERSION);
    ...
    /**
     * Returns the current context that is associated with the <b>calling thread</b>. GuldenJ is an API that has thread
     ...
     * independently use guldenj (or possibly alt coin forks of it) can operate correctly.
     *
    ...
    public static Context get() {
        ...
        if (tls == null) {
            if (isStrictMode) {
                log.error("Thread is missing a guldenj context.");
                log.error("You should use Context.propagate() or a ContextPropagatingThreadFactory.");
                throw new IllegalStateException("missing context");
            }
            if (lastConstructed == null)
                throw new IllegalStateException("You must construct a Context object before using guldenj!");
            slot.set(lastConstructed);
            log.error("Performing thread fixup: you are accessing guldenj via a thread that has not had any context set on it.");
    ...
    /**
     * Require that new threads use {@link #propagate(Context)} or {@link org.guldenj.utils.ContextPropagatingThreadFactory},
     ...
     * want to create core GuldenJ objects. Generally, if a class can accept a Context in its constructor and might
     * be used (even indirectly) by a thread, you will want to call this first. Your task may be simplified by using
     * a {@link org.guldenj.utils.ContextPropagatingThreadFactory}.
     */
    ...
    /**
     * Returns the {@link org.guldenj.core.NetworkParameters} specified when this context was (auto) created. The
