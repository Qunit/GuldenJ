...
import org.guldenj.params.AbstractBitcoinNetParams;
 ...
 *
 * <ul>
 * <li>{@code gulden:<address>}</li>
 * <li>{@code gulden:<address>?<name1>=<value1>&<name2>=<value2>} with multiple
 ...
 */
public class GuldenURI {
    /**
     * Provides logging for this class
     */
    private static final Logger log = LoggerFactory.getLogger(GuldenURI.class);
    ...
    /**
     * URI for Bitcoin network. Use {@link org.guldenj.params.AbstractBitcoinNetParams#BITCOIN_SCHEME} if you specifically
     * need Bitcoin, or use {@link org.guldenj.core.NetworkParameters#getUriScheme} to get the scheme
    ...
    /**
     * Constructs a new GuldenURI from the given string. Can be for any network.
     *
     * @param uri The raw URI data to be parsed (see class comments for accepted formats)
     * @throws GuldenURIParseException if the URI is not syntactically or semantically valid.
     */
    public GuldenURI(String uri) throws GuldenURIParseException {
        this(null, uri);
    }
     ...
     *
     * @throws GuldenURIParseException If the input fails Bitcoin URI syntax and semantic checks.
     */
    public GuldenURI(@Nullable NetworkParameters params, String input) throws GuldenURIParseException {
        checkNotNull(input);
        ...
        //Handle old 'guldencoin' URI's from old wallets etc.
        input = input.replace("uldencoin:","ulden:");
        ...
        try {
            ...
            throw new GuldenURIParseException("Bad URI syntax", e);
        }

        // URI is formed as  gulden:<address>?<query parameters>
        // blockchain.info generates URIs of non-BIP compliant form gulden://address?....
        ...
        // For instance with : gulden:129mVqKUmJ9uwPxKJBnNdABbuaaNfho4Ha?amount=0.06&label=Tom%20%26%20Jerry
        ...
        if (input.toLowerCase().startsWith(blockchainInfoScheme)) {
            ...
            throw new GuldenURIParseException("Unsupported URI scheme: " + uri.getScheme());
        }

        // Split off the address from the rest of the query parameters.
        String[] addressSplitTokens = schemeSpecificPart.split("\\?", 2);
        if (addressSplitTokens.length == 0)
            throw new GuldenURIParseException("No data found after the gulden: prefix");
        ...
        if (!addressToken.isEmpty()) {
            ...
            try {
                ...
                throw new GuldenURIParseException("Bad address", e);
            }
        }

        if (addressToken.isEmpty() && getPaymentRequestUrl() == null) {
            throw new GuldenURIParseException("No address and no r= parameter found");
        }
    }
     ...
     */
    private void parseParameters(@Nullable NetworkParameters params, String addressToken, String[] nameValuePairTokens) throws GuldenURIParseException {
        // Attempt to decode the rest of the tokens into a parameter map.
        for (String nameValuePairToken : nameValuePairTokens) {
            final int sepIndex = nameValuePairToken.indexOf('=');
            if (sepIndex == -1)
                throw new GuldenURIParseException("Malformed Bitcoin URI - no separator in '" +
                        nameValuePairToken + "'");
            if (sepIndex == 0)
                throw new GuldenURIParseException("Malformed Bitcoin URI - empty name '" +
                        nameValuePairToken + "'");
            ...
            if (FIELD_AMOUNT.equals(nameToken)) {
                ...
                try {
                    ...
                    if (params != null && amount.isGreaterThan(params.getMaxMoney()))
                        throw new GuldenURIParseException("Max number of coins exceeded");
                    if (amount.signum() < 0)
     ...
     */
    private void putWithValidation(String key, Object value) throws GuldenURIParseException {
        if (parameterMap.containsKey(key)) {
            throw new GuldenURIParseException(String.format(Locale.US, "'%s' is duplicated, URI is invalid", key));
        } else {
     ...
     *         10,000,000 units is 1 NLG. May be null if no amount is specified
     */
    ...
    public String toString() {
        StringBuilder builder = new StringBuilder("GuldenURI[");
        boolean first = true;
    ...
    public static String convertToGuldenURI(Address address, Coin amount, String label, String message) {
        return convertToGuldenURI(address.toString(), amount, label, message);
    }
     ...
     */
    public static String convertToGuldenURI(String address, @Nullable Coin amount, @Nullable String label,
